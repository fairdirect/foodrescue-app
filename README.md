...
* **📁 `lib/`:** External libraries needed by the project that cannot be provided by package managers and have to be installed manually. They are not included in the project's Git repository because that would be redundant – the libraries are managed in their own repositories somewhere already. On top of that, some folders in here will be Git repositories themselves, creating a mess if we'd include them into the main repository as well.

    Among others, all Android libraries except Qt have to be placed here in order to create the Android APK successfully. This is [due to a limitation of `qmlimportscanner`](https://stackoverflow.com/a/62326971), the program that runs to find all QML files that have to be included into the APK package. It can only scan a single directory recursively and does not scan imported QML files for their imports if located out of that single directory. It also does not follow symlinks to other directories. So all code that uses Qt QML files, including libraries, has to reside under the project's directory at compile time.
